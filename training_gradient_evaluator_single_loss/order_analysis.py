...
def analyze_and_plot(model_csv: str, imagenet_csv: str) -> None:
	...
	# Second plot: x = total loss to get correct, y = universal absolute rank (1..N)
	...
	if X2.std() > 0 and Y2.std() > 0:
		...
		corr2 = float('nan')
	plot_path2 = os.path.join(out_dir, "order_loss_vs_universal.png")
	plt.figure(figsize=(7, 6))
	plt.scatter(X2, Y2, s=8, alpha=0.5)
	plt.xlabel("Total Loss to Get Correct")
	plt.ylabel("Universal Difficulty Ranking (1=easiest)")
	plt.title(f"Universal Difficulty vs Total Loss to Get Correct\nOverlap size={M}  |  Pearson r={corr2:.4f}")
	plt.tight_layout()
	plt.savefig(plot_path2, dpi=150)
	plt.close()
	print(f"Saved loss-vs-universal plot to {plot_path2}")

	# Third plot (replacement): scatter of total loss to get correct vs GLOBAL difficulty rank
	plot_path3 = os.path.join(out_dir, "order_loss_vs_global_rank.png")
	plt.figure(figsize=(7, 6))
	plt.scatter(X2, Y2, s=8, alpha=0.5)
	plt.xlabel("Total Loss to Get Correct")
	plt.ylabel("Universal Difficulty Ranking (1=easiest)")
	plt.title("Universal Difficulty vs Total Loss to Get Correct")
	plt.tight_layout()
	plt.savefig(plot_path3, dpi=150)
	plt.close()
	print(f"Saved loss-vs-global-rank plot to {plot_path3}")


def _default_paths() -> Tuple[str, str]:
	# Programming root is parent of this file's parent
	root = Path(__file__).resolve().parents[1]
	model_csv = str(root / "training_gradient_evaluator" / "outputs" / "resnet34.a3_in1k" / "first_correct_summary.csv")
